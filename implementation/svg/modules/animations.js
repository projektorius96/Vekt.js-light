...
const { Converters } = Helpers.Trigonometry;
...
function Crossproduct$part2({reverseCountOnCondition, count, paths, Helpers, UnitVector, GROW_ALONG_SLOPE, GLOBAL_SCALAR, XMLSVG, ENUMS}) {
    ...
    try {
        ...
        */
        ...
        // DEV_NOTE # update y-axis visual (rotate arrow). UnitVector.drawVector will set transform using the passed `angle`.
        if (paths.y_axis) {
        ...
        // DEV_NOTE # now update parallelogram (i.e. unit_square) so it follows x/y axes each frame.
        ...
        if (unitSquarePath && xAxisPath && yAxisPath) {
            ...
            // DEV_NOTE # get Current Transformation Matrix (CTM) use Vekt.js-first API method of `SVGPathElement.getCurrentMatrix()`
            const mx = xAxisPath.getCurrentMatrix();
            const my = yAxisPath.getCurrentMatrix();
            const mPar = unitSquarePath.getCurrentMatrix();
            ...
            if (mx && my && mPar) {
                ...
                // normalized optional dot and herein mandatory cross (2D scalar cross)
